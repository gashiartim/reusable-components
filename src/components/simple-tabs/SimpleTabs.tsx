...
import TabLabel, { ITabLabel } from 'shop/client/components/molecules/simple-tabs/tab-label/TabLabel';
...
import Tab, { ITab } from 'shop/client/components/molecules/simple-tabs/tab/Tab';
...
const SimpleTabs: FunctionComponent<ISimpleTabs> & ISimpleTabsChildrens = (props): JSX.Element => {
    const { children, initialActiveTab, contentClassName } = props;
    const [activeTab, setActiveTab] = useState<number>(() => initialActiveTab || 0);

    const contentClassNames = classNamesHelper([Classes['content'], contentClassName]);
    ...
    function renderLabels() {
        return children.map((child, index) => {
            ...
            return child.props.show && child.type === Tab ? (
                <TabLabel key={`tab-label-${index}`} {...child.props} index={index} setActiveTab={activeTabHandler} isActive={index === activeTab} />
            ) : null;
        });
    }
